...
3) Read the "macc_resume.md" file to understand the project context + The generated code must be commented.
...
invoke_ai() {
  ...
  set +e
  printf "%s" "$prompt_text" | codex --model $AI_MODEL --sandbox "$sandbox" --ask-for-approval "$approval" -C "$repo_root" exec - 2>&1 \
    | tee "$log_file" "$tmp_out"
  local codex_status="${PIPESTATUS[1]}"
  set -e

  if [[ "$codex_status" -ne 0 ]]; then
    if grep -qiE 'unknown (flag|option).* -C|unrecognized option.*-C|flag provided but not defined' "$tmp_out"; then
      set +e
      ( cd "$repo_root" && printf "%s" "$prompt_text" | codex --model $AI_MODEL --sandbox "$sandbox" --ask-for-approval "$approval" exec - ) 2>&1 \
        | tee "$log_file" "$tmp_out"
...
extract_assistant_section() {
  awk '
    ...
    /^[[:space:]]*codex[[:space:]]*$/ { seen=1; buf=""; next }
    { if (seen) buf = buf $0 "\n"; else full = full $0 "\n" }

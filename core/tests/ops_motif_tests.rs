...
use macc_core::ops_motif::{
    apply_preset_to_config, apply_preset_to_env_cfg, calculate_trust_summary,
    generate_lock_manifest, get_setting_descriptors, verify_lock_manifest, SettingCategory,
    TrustState,
};
...
fn test_setting_descriptors_exist_and_categorized() {
    ...
    let has_basic = descriptors
        .iter()
        .any(|d| d.category == SettingCategory::Basic);
    let has_advanced = descriptors
        .iter()
        .any(|d| d.category == SettingCategory::Advanced);
    let has_admin = descriptors
        .iter()
        .any(|d| d.category == SettingCategory::Admin);

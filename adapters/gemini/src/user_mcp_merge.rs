...
pub fn plan_user_mcp_merge(plan: &mut ActionPlan, servers: &BTreeMap<String, Value>) -> Result<()> {
    ...
    let user_config = home.join(".gemini/settings.json");
    let user_path = user_config.to_string_lossy().into_owned();

    let existing = read_user_gemini_json(&user_config, &user_path)?;
...
fn read_user_gemini_json(path: &Path, display_path: &str) -> Result<Value> {
    if !path.exists() {
    ...
    let content = fs::read_to_string(path).map_err(|e| MaccError::Io {
        ...
        action: "read user Gemini config".into(),
        source: e,
    })?;
...
mod tests {
    ...
    fn adds_missing_servers_and_preserves_existing() {
        with_temp_home(|home| {
            let user_file = home.join(".gemini/settings.json");
            ...
            assert!(
                matches!( plan.actions.get(0), Some(Action::BackupFile { path, .. }) if path.ends_with(".gemini/settings.json"))
            );

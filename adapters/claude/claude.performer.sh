...
usage() {
  cat <<'EOF'
Usage:
  claude.performer.sh --prompt-file <path> --tool-json <path> [--repo <path>] [--worktree <path>] [--task-id <id>] [--attempt N] [--max-attempts N]
EOF
}
...
run_default_call() {
  ...
  if [[ "$prompt_mode" == "arg" ]]; then
    ...
    local rc=0
    printf '[MACC] invoke (claude 2): %s' "$command" >&2

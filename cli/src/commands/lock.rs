    ...
    fn run(&self) -> Result<()> {
        ...
        match self.subcommand {
            LockCommands::Generate => {
                ...
                let serialized = serde_yaml::to_string(&lock).map_err(|e| {
                    macc_core::MaccError::Validation(format!("serialize lock manifest: {}", e))
                })?;
                fs::write(&lock_path, serialized).map_err(|e| macc_core::MaccError::Io {
            ...
            LockCommands::Check => {
                if !lock_path.exists() {
                    return Err(macc_core::MaccError::Validation(
                        "Lock file not found. Run 'macc lock generate' first.".to_string(),
                    ));
                }
                println!("Checking environment lock integrity...");
                let lock_str =
                    fs::read_to_string(&lock_path).map_err(|e| macc_core::MaccError::Io {
                        path: lock_path.to_string_lossy().into(),
                ...
                if report.matches {
                    ...
                    return Err(macc_core::MaccError::Validation(
                        "Lock file check failed due to drift.".to_string(),
                    ));
                }
            }
            LockCommands::Diff => {
                if !lock_path.exists() {
                    println!(
                        "Lock file does not exist yet. Current environment represents 100% drift."
                    );
                    return Ok(());
                }
            ...
            LockCommands::Explain => {
                ...
                println!(
                    "- tools: list of resolved AI coding tools and their generated file hashes"
                );

...
pub(crate) struct ApiPlanRequest {
    ...
    /// Worktree IDs selected in the UI for future plan scoping.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub worktrees: Vec<String>,
    /// Whether user-scope operations are allowed into the plan.
    pub allow_user_scope: Option<bool>,
    /// Whether planning should avoid remote access regardless of config.
    pub offline: Option<bool>,
...
pub(crate) struct ApiPlanResponse {
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub risks: Vec<ApiPlanRisk>,
    /// Explicit consent prompts required by the plan.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub consents: Vec<ApiPlanConsent>,
}

/// Shared risk classification used by plan previews.
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "kebab-case")]
pub(crate) enum ApiRiskLevel {
    Safe,
    Caution,
    Dangerous,
}
...
pub(crate) struct ApiPlanFile {
    ...
    /// Classified risk for the operation.
    pub risk_level: ApiRiskLevel,
    /// Short serialized preview of the planned content when available.
...
/// Risk notice generated by a plan preview.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub(crate) struct ApiPlanRisk {
    /// Classified severity used by the UI.
    pub level: ApiRiskLevel,
...
pub(crate) struct ApiPlanConsent {
    ...
    pub scope: Scope,
    /// Classified severity for the consent gate.
    pub classification: ApiRiskLevel,
